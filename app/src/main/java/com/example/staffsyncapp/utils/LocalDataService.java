...
public class LocalDataService extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE notification_preferences (" +
                "id INTEGER PRIMARY KEY AUTOINCREMENT," +
                "user_id INTEGER UNIQUE," +
                "leave_notifications INTEGER DEFAULT 1," + // boolean for leave updates
                "salary_notifications INTEGER DEFAULT 1," + // boolean for salary updates
                "email_notifications INTEGER DEFAULT 1," +  // boolean for email updates
                "FOREIGN KEY(user_id) REFERENCES users(id))");
        ...
        try {
            ...
            userValues.put("password", hashPassword("user123"));
            userValues.put("is_admin", 0); // not admin
            long userInsertResult = db.insertOrThrow("users", null, userValues);
            Log.d("StaffDataService", "Test user account created with ID: " + userInsertResult);
        } catch (Exception e) {
            Log.e("StaffDataService", "Error creating test user: " + e.getMessage());
        }
    }
    ...
    // check if admin is logged in; use
